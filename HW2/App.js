...
function App() 
{
  ...
  //const[todos,settodos] = useState(initialtodos)
    ...
    function Testtodo(state, action) {
      switch (action.type) {
        case "CREATE_POST":
          const newpost = {
            title: action.title,
    ...
    const [todos, todosdispatcher] = useReducer(Testtodo, initialtodos);
    
    const handleaddtodo = (newpost) => 
    {
      todosdispatcher({type:"CREATE_POST", ...newpost});
    }
  return (
    <div>
       ...
       <Createtodo user={user} handleaddtodo={handleaddtodo} todos={todos} />
