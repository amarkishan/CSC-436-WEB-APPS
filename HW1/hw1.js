...
const Product = class Product 
{
    ...
    {
        return products.map(product => ({ ...product, price: product.price / 2 }));      
    }
    ...
    static printProducts = (products) => {
        ...
        for(const product of products )
        {
            const availabilitytest = product.availability === "In Stock" ? "Yes":"No";
            const formattedprice = parseFloat(product.price).toFixed(2);
            console.log(`Product: ${product.name}, Cost: $${formattedprice}, Availability: ${availabilitytest}`);
        }
    }
